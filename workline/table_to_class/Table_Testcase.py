...
import sys
sys.path.append('/root/asmfuzz/workline')

from src.studyMysql.Table_Operation import Table_Testcase, Table_Function
from harness_tools.harness_class import Harness as H_java
from harness_tools.harness_class_javac_copy import Harness as H_javac
...
class Testcase_Object(object):
    ...
    def check_classfile(self) -> int:
        ...
        if self.Testcase_context != "":
            ...
            logger.info('*' * 25 + "start check class file...")
    ...
    def single_engine_compile_coverage(self, want_testbed_location, mm):
        harness = H_javac()
        logger.info("*" * 25, "Compiling the ", self.Id, "testcase...", "*" * 25)
        if self.Testcase_context != "":
            harness.get_class_name(self.Testcase_context)
            name = harness.name
