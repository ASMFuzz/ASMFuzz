...
from workline.harness_tools.harness_class_javac_copy import Harness as H_javac
...
class Coverage:
    ...
    def check(self, testcase,mm) -> int:
        counter  = 0
        code_files = "/root/asmfuzz/fuzz/shannon_v0/"+str(mm)+"/"
        harness = H_javac()
...
if __name__ == '__main__':
    mm = 3  # NUM
    docpath = "/root/asmfuzz/fuzz/coverage/mm"+str(mm)+"/coverageAll.txt"

    obj1 = Coverage()
    obj2 = Energy()
    c = Counter()

    count_record = "/root/asmfuzz/fuzz/coverage/countHistory.txt"
    countHistoryDir = '/root/asmfuzz/fuzz/coverage/countHistoryDir'+str(mm)+'/'
    obj2.val_dir = "/root/asmfuzz/feature/src/test/output/target_test"
    obj2.Mutation_method = mm
    all_val_files_data = obj2.read_mutators_fromDir()[str(mm)]

    tempfile = "/root/asmfuzz/fuzz/coverage/temp.txt"
    ...
    for index in range(0, len(obj1.groupList)):
        ...
        script_directory = "/root/asmfuzz/fuzz/scripts"
        script_name = "time_coverage.sh"
        arg1_logfile = "/root/asmfuzz/fuzz/coverage/mm"+str(mm)+"/log.txt"
        arg2_mm = str(mm)
        arg3_index = str(index)
        subprocess.run(['bash', script_name, arg1_logfile, arg2_mm, arg3_index], cwd=script_directory)

        allFilepath = "/root/asmfuzz/fuzz/coverage/mm"+str(mm)+"/all"+str(index)+".txt"
        ...
        html_root_dir = '/root/asmfuzz/fuzz/coverage/mm'+str(mm)+'/HtmlFile'+str(index)
        analyse_output_path = '/root/asmfuzz/fuzz/coverage/mm'+str(mm)+'/analyse'+str(index)+'.txt'
