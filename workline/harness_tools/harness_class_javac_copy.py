...
import os
import sys
basePath = "/root/asmfuzz/workline"
...
configPath = os.path.join(basePath,"config.yml")

with open(configPath, 'r', encoding='utf-8') as f:
    config = yaml.load(f.read(), Loader=yaml.FullLoader)
# BASE_DIR = str(Path(__file__).resolve().parent.parent.parent)
listTxt = config['harness_class']['listTxt']
code_files = config['harness_class']['code_files']
code_files_cov = config['harness_class']['code_files_cov']
addExportsPath = config['harness_class']['addExportsPath']
...
class Harness:
    ...
    def run_testcase(self, function_id: int, testcase_id: int, testcase_context: str) -> HarnessResult:
        ...
        # result.outputs = self.single_thread(self.name,testcase_context)
        return result
