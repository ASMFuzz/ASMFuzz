...
public class checkGenerate {
    ...
    public static List<String> check(String fileName,String targetFileName) {
        ...
        List<String> options = new ArrayList<String>(Arrays.asList("-classpath", "/root/asmfuzz/parser/generate_tools/Dependencies", "-d", "/root/asmfuzz/parser/generate_tools/TmpClasses"));
        ...
        File passFile = new File("/root/asmfuzz/seedData/pass_data/"+targetFileName);
        if(!passFile.exists()){
            passFile.mkdir();
        }

        File targetFile = new File("/root/asmfuzz/seedData/pass_data/"+targetFileName+"/"+fileName.substring(fileName.lastIndexOf("/")+1));
        if(messages.isEmpty()){
    ...
    public static void main(String[] args){
        ...
        String absolutePath = "/root/asmfuzz";
