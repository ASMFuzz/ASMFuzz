...
//    int number = 1;
    ...
    public Statement Insert_Cov(List<CompilationUnit> Seed_compilations, String FilePath, String Seed_file) throws Exception {
        int file_num = 0;
        Map<String, List<CompilationUnit>> seed_Map = this.groupList(Seed_compilations); //将种子文件分组，每组100个文件，每次变异一组
//        System.out.println("grouped success: total " + seed_Map.size() + " group!");
        for (Map.Entry<String, List<CompilationUnit>> entry_com : seed_Map.entrySet()) {
            //cu(List)种子cu，filepath+num createfile参数,seedpath+num createfile参数
            List<CompilationUnit> temp = new ArrayList<>(entry_com.getValue());
            Insert(temp, FilePath + "/" + file_num, Seed_file + "/" + file_num);
            file_num++;
        }
//        System.out.println("group num :" + seed_Map.size());
...
//        List<MethodCallExpr> unwanted = new ArrayList<>();
...
//                            System.out.println("Java parse success");
                        ...
                        deletevalThis(test);
//                        System.out.println("cur filepath:" + name_1);
//                        System.out.println("cur insertStmt:" + test);
...
//                System.out.println("Directory created: " + path);
    ...
    public static void rmDir(String folderPath){
        ...
        if (folder.exists() && folder.isDirectory()) {
            ...
            System.out.println("The specified path is not a folder or the folder does not exist.");
        }
    }

    public static int FolderNumber(String directoryPath){
            File directory = new File(directoryPath);
            File[] files = directory.listFiles();
            int maxNumber = 0;

            for (File file : files) {
                if (file.isDirectory() && file.getName().startsWith("folder_")) {
                    String folderName = file.getName().substring(7);
                    try {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String pathAll = "/root/asmfuzz/seedData/pass_data/0";
            String taskNumber = "003";
            String directoryPath = String.format("/root/asmfuzz/parser/feature/src/test/output/task%s",taskNumber);
            int newNumber = FolderNumber(directoryPath);
            String folderName = "folder_" + newNumber;
            String seedpath = String.format("/root/asmfuzz/parser/feature/src/test/output/task%s/%s/seedTemp_testsuits", taskNumber,folderName);
            String filepath = String.format("/root/asmfuzz/parser/feature/src/test/output/task%s/%s/target_testsuits", taskNumber,folderName);

            createDirectoryIfNotExists(seedpath);
