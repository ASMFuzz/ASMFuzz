...
public class VariableGenerater {
    ...
    public String generateValueOfDec2(InsertVal curObj,String type){
        ...
        String iid = "";
        try {
//            String query = "SELECT * FROM variables WHERE vari_type = '" + type + "' ORDER BY energy DESC LIMIT 1;";
            String query = "SELECT * FROM variables WHERE vari_type = '" + type + "' ORDER BY RAND() LIMIT 1;";
            ResultSet rs = stmt.executeQuery(query);
            List res = convertList2(rs);
            if(res.size()>1) {
                Random r = new Random();
                int setindex = r.nextInt(res.size() - 1);
...
//            curObj.name_and_type_updateNeedSetValName.add(new HashMap<>());
