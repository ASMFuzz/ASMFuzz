...
//                        if (!(list_avar_key.get(index)).equals(list_need_key.get(index_need))) {
                    ...
                    //变量类型不相同时
                        ...
                        //类型不同，但名称相同
                        if (list_avar_key.get(index).equals(list_need_key.get(index_need))) {
                            //在待插入语句中，将变量名称修改为oldname+"_change"的形式
