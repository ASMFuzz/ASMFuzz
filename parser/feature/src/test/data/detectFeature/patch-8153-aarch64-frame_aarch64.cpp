...
@@ -271,6 +274,7 @@ void frame::patch_pc(Thread* thread, address pc) {
...
@@ -280,30 +284,24 @@ void frame::patch_pc(Thread* thread, address pc) {
     tty->print_cr("");
   }
...
@@ -330,7 +328,7 @@ BasicObjectLock* frame::interpreter_frame_monitor_begin() const {
 }
...
@@ -405,6 +403,7 @@ void frame::verify_deopt_original_pc(CompiledMethod* nm, intptr_t* unextended_sp
...
@@ -416,32 +415,12 @@ void frame::adjust_unextended_sp() {
...
+#endif
 ...
 //------------------------------------------------------------------------------
@@ -453,6 +432,7 @@ frame frame::sender_for_interpreter_frame(RegisterMap* map) const {
...
+  intptr_t* sender_fp = link();
   ...
   if (map->update_map()) {
@@ -463,94 +443,15 @@ frame frame::sender_for_interpreter_frame(RegisterMap* map) const {
...
@@ -659,7 +560,6 @@ BasicType frame::interpreter_frame_result(oop* oop_result, jvalue* value_result)
   return type;
 }
...
@@ -682,6 +582,14 @@ void frame::describe_pd(FrameValues& values, int frame_no) {
...
@@ -690,19 +598,6 @@ intptr_t *frame::initial_deoptimization_info() {
   return NULL;
 }
