...
+import static sun.security.ssl.JsseJce.CIPHER_RC4;
 ...
 enum SSLCipher {
     // exportable ciphers
@@ -881,39 +891,43 @@ static final class StreamReadCipher extends SSLReadCipher {
...
+                        -1, -1L, pt.slice());
             ...
             @Override
@@ -1059,26 +1073,28 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
...
-                    }
                     ...
                     // catch BouncyCastle buffering error
                     throw new RuntimeException("Cipher buffering error in " +
@@ -1088,14 +1104,14 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
                 if (SSLLogger.isOn && SSLLogger.isOn("plaintext")) {
                     SSLLogger.fine(
...
+                        protocolVersion);
                     ...
                     if (reservedBPE == null) {
                         reservedBPE = bpe;
@@ -1106,7 +1122,7 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
                 // block cipher suites.
                 try {
                     if (tagLen != 0) {
...
+                        checkCBCMac(signer, pt,
                         ...
                         authenticator.increaseSequenceNumber();
@@ -1124,7 +1140,7 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
...
+                        -1, -1L, pt.slice());
             ...
             @Override
@@ -1330,48 +1346,49 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
                 int cipheredLength = bb.remaining();
                 int tagLen = signer.macAlg().size;
                 if (tagLen != 0) {
-                    if (!sanityCheck(tagLen, bb.remaining())) {
...
+                    removePadding(pt, tagLen, blockSize, protocolVersion);
                     ...
                     if (reservedBPE == null) {
                         reservedBPE = bpe;
@@ -1382,7 +1399,7 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
                 // block cipher suites.
                 try {
                     if (tagLen != 0) {
-                        checkCBCMac(signer, bb,
+                        checkCBCMac(signer, pt,
                         ...
                         authenticator.increaseSequenceNumber();
@@ -1400,7 +1417,7 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
...
+                        -1, -1L, pt.slice());
             ...
             @Override
@@ -1655,10 +1672,19 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
...
+                    len = cipher.doFinal(bb, pt);
                     ...
                     // unlikely to happen
                     throw new RuntimeException(
@@ -1670,17 +1696,17 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
                         "JCE provider " + cipher.getProvider().getName(), sbe);
                 }
...
+                        -1, -1L, pt.slice());
             ...
             @Override
@@ -1922,17 +1948,24 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
                     throw new RuntimeException(
...
+                    len = cipher.doFinal(bb, pt);
                     ...
                     // unlikely to happen
                     throw new RuntimeException(
@@ -1944,32 +1977,31 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
                         "JCE provider " + cipher.getProvider().getName(), sbe);
                 }
                 // reset the limit to the end of the decrypted data
-                bb.position(pos);
...
+                        -1, -1L, pt.slice());
             ...
             @Override
@@ -2203,11 +2235,19 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
...
+                    len = cipher.doFinal(bb, pt);
                     ...
                     // unlikely to happen
                     throw new RuntimeException(
@@ -2219,17 +2259,17 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
                         "JCE provider " + cipher.getProvider().getName(), sbe);
                 }
                 // reset the limit to the end of the decrypted data
-                bb.position(pos);
...
+                        -1, -1L, pt.slice());
             ...
             @Override
@@ -2473,11 +2513,18 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
                                         contentType, bb.remaining(), sn);
                 cipher.updateAAD(aad);
...
+                    len = cipher.doFinal(bb, pt);
                     ...
                     // unlikely to happen
                     throw new RuntimeException(
@@ -2489,29 +2536,29 @@ public Plaintext decrypt(byte contentType, ByteBuffer bb,
                         "JCE provider " + cipher.getProvider().getName(), sbe);
                 }
                 // reset the limit to the end of the decrypted data
-                bb.position(pos);
