...
logfile = os.path.join(script_dir, '02error_constructor.log')

class ClassInfo:
    """all classes information"""

    def __init__(self, class_name: str, class_description: str, class_constructor: str, package_name: str):
    ...
    def format_output(self) -> List[str]:
        format_info = [self.class_name, self.class_description, self.class_constructor, self.package_name]
        # print(format_info)
        return format_info
...
def get_class_description(soup: BeautifulSoup) -> str:
    desc_div = soup.find('div', class_='description')
    return desc_div.pre.get_text()
...
if __name__ == '__main__':
    ...
    try:
        ...
        while index < len(all_classes_info):
            ...
            package_name = soup.find_all('div', class_='subTitle')[-1].get_text()
            ...
            if all_constructor_info == "":  # this class has no generatable constructor
                ...
                class_description = get_class_description(soup)
                all_format_input.append(
                    ClassInfo(class_name, class_description, all_constructor_info, package_name).format_output())
                success_num += 1
            index += 1
            pbar.update(1)
    except:
        ...
        sql = "insert into Constructor_Info(Class_Name,Class_Description,Constructor_Stmt,Package_Name) values(%s,%s,%s,%s)"
