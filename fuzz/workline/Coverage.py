...
class Coverage:
    ...
    # 0. Execute test cases, then execute the script [time_coverage.sh log_filename group_number] to calculate coverage.
    # 1. Based on the execution information recorded in all.txt by command 2 (genhtml_cmd), parse the overall coverage value.
    ...
    # 2. Parse the entire HTML directory (HtmlFile${NUM}) generated by command 2, and record it in the file of output_path.
    ...
    # 3.2 After two rounds of parsing the HTML directory (HtmlFile${NUM}), obtained the files analyse1.txt and analyse2.txt, compare to see if new coverage has been generated.
    # Get new coverage
    # params: test1f, test2f are the results read from the Html directory after two rounds of test coverage, i.e., the second parameter of analyseHtmls.
    ...
    def check(self, testcase,mm) -> int:
        counter  = 0
        code_files = "/root/shannonfuzz-python/fuzz/shannon_v0/"+str(mm)+"/"
        harness = H_javac()
...
if __name__ == '__main__':
    mm = 1  # NUM
    docpath = "/root/shannonfuzz-python/fuzz/coverage/mm"+str(mm)+"/coverageAll.txt"

    obj1 = Coverage()
    obj2 = Energy()
    c = Counter()

    count_record = "/root/shannonfuzz-python/fuzz/coverage/countHistory.txt"
    countHistoryDir = '/root/shannonfuzz-python/fuzz/coverage/countHistoryDir'+str(mm)+'/'
    # 3.1.3 Recursively traverse the test9_1 directory (including subdirectories) [val_dir], read the specified mutation operator database and ID number related to the test cases for [Mutation_method].
    # obj2.val_dir = "/root/shannonfuzz-python/fuzz/data/test"+str(mm)+"_1/"
    obj2.val_dir = "/root/shannonfuzz-getfeature/feature/src/test/output/target_test"
    obj2.Mutation_method = mm
    all_val_files_data = obj2.read_mutators_fromDir()[str(mm)]
    # print(all_val_files_data)

    tempfile = "/root/shannonfuzz-python/fuzz/coverage/temp.txt"

    #1.Execute test cases.
    obj1.number = 15 # How many test cases per group?
    obj1.getListTestcase(mm)    #Grouping.
    # print(obj1.groupList)
    print("There are ",len(obj1.groupList)*obj1.number," test cases in the database!")

    #2. Calculate coverage and make records.
    for index in range(0, len(obj1.groupList)):
        obj1.cur_group_names = []
        # print(len(obj1.groupList[index][0]))
        obj1.javac_Testcases(obj1.groupList[index],mm)
        obj1.run_Testcases(obj1.groupList[index],mm)
        script_directory = "/root/shannonfuzz-python/fuzz/scripts"
        script_name = "time_coverage.sh"
        arg1_logfile = "/root/shannonfuzz-python/fuzz/coverage/mm"+str(mm)+"/log.txt"
        arg2_mm = str(mm)
        arg3_index = str(index)
        subprocess.run(['bash', script_name, arg1_logfile, arg2_mm, arg3_index], cwd=script_directory)

        allFilepath = "/root/shannonfuzz-python/fuzz/coverage/mm"+str(mm)+"/all"+str(index)+".txt"
        # print(allFilepath)
        ...
        #3. Process the HTML directory to obtain the corresponding analysis document.
        html_root_dir = '/root/shannonfuzz-python/fuzz/coverage/mm'+str(mm)+'/HtmlFile'+str(index)
        analyse_output_path = '/root/shannonfuzz-python/fuzz/coverage/mm'+str(mm)+'/analyse'+str(index)+'.txt'
        start_time = time.time()
        obj1.analyseHtmls(html_root_dir, analyse_output_path)
        print(f'analyse Htmls Spending time:{int(time.time() - start_time)}s')
        #3.1.1 Based on the "analyse.txt" file, calculate the execution frequency for each line and each branch recorded in the file.
        obj2.get_frequency(analyse_output_path)
        # prefix = html_root_dir
        with open(analyse_output_path, 'r') as f1:
            analyse_con = f1.readlines()
        analyse_files = obj2.get_file_info(analyse_con)

        # # Align the calculation results with the previous countHistory.
        ...
        #3.1.2 Calculate the entropy based on the Counter result.
        energy = obj2.get_energy(count_res)

        # 3.1.3 finished
        # 3.1.4 Write back the entropy value to the mutation operator database "energy".
        ...
        # Update and write back the statistical results.
    ...
    # res = obj.parse_coverage_info("/root/shannonfuzz-python/fuzz/coverage/ALL.txt")
    ...
    # obj.analyseHtmls(root_dir,output_path)


    # prefix1 = "/JVMfuzzing/wy/HtmlFile3" 
    # prefix2 = "/JVMfuzzing/wy/HtmlFile3"  
    # test1f="/JVMfuzzing/wy/analyse1.txt"
    # test2f="/JVMfuzzing/wy/analyse2.txt"
    # newFile,newLines,newBranches = obj.get_new_coverage(test1f,test2f,prefix1,prefix2)
    # print(newFile)
    # print(newLines)
    # print(newBranches)
