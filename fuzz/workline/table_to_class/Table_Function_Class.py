...
class Function_Object(object):
    ...
    def gpt_mutation_1_2(self, sess, if_save_function):
        ...
        print("1. Currently using GPT for continuation and GPT for substitution mutation.")
        ...
        for prefix_line in range(self.var_line_count, self.js_line_count):
            ...
            try:
                ...
                print('Prefix is too long, skipping.')
                continue

            all_functions_pass = self.jshint_check_function(all_functions)

            # Mutation Method 2: Code block replacement.
            if IfReplaceBlock:
                # Number of lines in the GPT prefix, ranging from 1 to gpt_line_num.
            ...
            if if_save_function:
                print(
                    f"Generated {len(all_functions_pass)} GPT continuation functions, generated {len(all_functions_replace_block)} GPT replace block functions, total time elapsed: {int(end_time - start_time)} seconds.")
        ...
        print(
            f"Generated {len(all_functions_generated)} GPT continuation test cases, generated {len(all_functions_replaced_generated)} GPT replace block functions, total time elapsed: {int(time.time() - all_start_time)} seconds.")
    ...
    def function_generate(self, function_prefix, sess):
        ...
        print("-" * 50 + f"\nSelected prefix: {function_prefix.strip()}")
    ...
    def gpt_mutation_3(self, sess):
        print("2. Currently using variable replacement mutation.")
        start_time = time.time()
        ...
        if var_number:
            ...
            print(
                f"Generated {len(all_functions_replace_block_pass)} GPT variable replacement test cases, total time elapsed: {int(end_time - start_time)} seconds.")

        else:
            print(f'Case {self.Id} has no variable definitions, skip variable replacement mutation.')
    ...
    def jshint_check_function(self, all_functions):
        start_time = time.time()
        print("Currently performing syntax check on the generated functions using jshint.")
        ...
        print(
            f"Generated a total of {len(all_functions)} functions, of which {len(all_functions_pass)} functions have correct syntax. Total time spent on the check: {int(end_time - start_time)} seconds.")
        return all_functions_pass

    def jshint_check_testcases(self, all_testcases):
        start_time = time.time()
        print("Currently performing syntax check on the generated test cases using jshint.")
        ...
        end_time = time.time()
        print(
            f"A total of {len(all_testcases)} test cases were assembled, of which {len(all_testcases_pass)} test cases have correct syntax. Total time spent on the check: {int(end_time - start_time)} seconds.")
    ...
    def assemble_to_testcase(self):
        ...
        try:
            ...
            for i in range(10):
                print("Currently performing assembly for the " + str(i) + "th time.")
