...
class ThreadLock(Thread):
    ...
    def run_test_case(self, testbed_location: str, testcase_path: pathlib.Path, testbed_id):
        ...
        try:
            print("start java...", testcase_path)
...
class Harness:
    ...
    # Retrieve the class name of the first class.
...
def run_test_case(testbed_location: str, testcase_path: pathlib.Path, testbed_id, time: str = "30"):
    cmd = "timeout -s9 " + time + " "
    # cmd = []
    ...
    if output is not None:
        ...
        classfilePath = code_files + testbed_location.split("/")[3] + "/out/" + testcase_path.name.replace(".java",
                                                                                                           ".class")
        print(classfilePath)
