...
if __name__ == '__main__':
    ...
    print("*" * 20, "3_1")
    # If the file is taken as a sample unit, and the frequency is measured by the number of hits on lines and branches, then it is meaningless to identify which file and line are being hit. The more evenly distributed the hits are among the files, the higher the entropy value.
    ...
    print("*" * 20, "3_2")
    # If we take lines of code as the sample unit, and measure frequency based on the number of hits on lines and the sum of branch counts on those lines, then the distribution of hit frequencies on lines will determine the change in entropy. If the frequencies of hits on lines correspond identically to increments and decrements, then the entropy will not change.
    # The more evenly distributed the frequency of hits on lines, the higher the entropy value.
