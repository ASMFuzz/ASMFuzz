...
class Energy:
    ...
    def __init__(self):
        ...
        self.val_dir = ""
        self.conn = dict(host='10.15.0.35',
                         port=10348,
                         user='root',
                         passwd='mysql123',
                         db='experiment',
    ...
    # Extract file information (3.1.1).
    ...
    # 3.1.1 After parsing an HTML directory (HtmlFile${NUM}), an analyse1.txt file is obtained. The file records the triggered lines and calculates the execution frequency of each branch.
    def get_frequency(self, analysef):
        with open(analysef, 'r') as f1:
            analyse1 = f1.readlines()
        analyse1_files = self.get_file_info(analyse1)
        c = Counter()
        # Method 1: Counting the returned result of the 'get_file_info' function.
        c.count(analyse1_files)
        str1 = c.get_counts()
        # print(str1)
        c2 = Counter()
        # Method 2: Counting based on the historical count information (converting the return value of 'get_counts' to a string) and continuing the counting process.
        c2.count_from_string(str(str1))
        # print(c2.get_counts())
        return c2.get_counts()

    # 3.1.2 After calculating the frequencies, it is necessary to calculate the entropy based on the results of the Counter.
    ...
    # 3.1.3 Recursively traverse the test9_1 directory (including subdirectories) and read the mutation operator database and ID numbers related to the test cases.
    ...
    # 3.1.4 Write the entropy values back to the mutation operator database 'energy'.
...
if __name__ == '__main__':
    obj = Energy()
    # 3.1.1 Based on the analyse1.txt file, calculate the execution frequency of each triggered line and each branch recorded in the file.
    ...
    # 3.1.2 Calculate the entropy based on the results from Counter.
    energy = obj.get_energy(c2)
    # print(energy)

    # 3.1.3 Recursively traverse the test9_1 directory (including subdirectories) and read the mutation operator database and ID numbers related to the specified Mutation_method test cases.
